...
export default function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  }, [transactions, search, category, eligibleOnly]);
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-2xl font-semibold tracking-tight text-navy-900">{account.nickname}</h1>
          ...
          <div className="text-right">
            <p className="text-3xl font-semibold tracking-tight text-navy-900 tnum">
      ...
      <section className="card p-5">
        <h2 className="mb-4 text-sm font-semibold text-navy-900">Transactions</h2>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          <div className="relative min-w-[12rem] flex-1">
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm outline-none transition focus:border-navy-500 focus:ring-2 focus:ring-navy-500/20"
            />
          </div>
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-navy-800 outline-none focus:border-navy-500"
          >
        ...
        {visible.length === 0 ? (
          <EmptyState title="No transactions" body="Nothing matches those filters." />
        ) : (
          <ul className="divide-y divide-slate-100">
            {visible.slice(0, 100).map((transaction) => (
...
function TransactionRow({ transaction }: { transaction: Transaction }) {
  ...
  const body = (
    <div className="-mx-2 flex items-center gap-3 rounded-lg px-2 py-3 transition hover:bg-slate-50">
      <span
        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
          transaction.amountCents > 0 ? 'bg-navy-50 text-navy-700' : merchantColor(name)
        }`}
        aria-hidden="true"
      >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium text-navy-900">{transaction.description}</p>
      ...
      <p
        className={`text-sm font-semibold tnum ${
          transaction.amountCents > 0 ? 'text-navy-700' : 'text-navy-900'
        }`}
      >
...
function CardControls({ account, onChange }: { account: AccountSummary; onChange: () => void }) {
  ...
  return (
    <section className="card p-6">
      <h2 className="text-sm font-semibold text-navy-900">Card controls</h2>

      <div
        className={`mt-4 rounded-2xl p-5 text-white transition-colors ${
          account.isLocked ? 'bg-navy-900' : 'bg-navy-700'
        }`}
      >
        ...
        {account.rewardsCents !== null && (
          <p className="mt-4 text-xs text-navy-200 tnum">
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
        ...
        <dl className="mt-2 space-y-1.5 text-xs leading-relaxed text-navy-600">
          <div className="flex gap-2">
            <dt className="w-32 shrink-0 font-medium text-navy-800">Stops</dt>
            <dd>New purchases, cash advances and balance transfers.</dd>
          </div>
          <div className="flex gap-2">
            <dt className="w-32 shrink-0 font-medium text-navy-800">Does not stop</dt>
            <dd>
